...
const FactorProgress = ({ name, goal, progress }: TargetAchievementFactor) => {
  const progressRate = goal === 0 ? 0 : Math.min((progress / goal) * 100, 100);
  ...
  return (
    <Stack spacing={3}>
      <Stack spacing={2.5}>
        ...
        <Stack spacing={0.4}>
          <Typography sx={{ color: ACCENT_COLOR, fontWeight: 900 }}>
            {title}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            条件を満たすと達成できます。
      ...
      <Stack spacing={1.25} sx={{ pb: 0.5 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          ...
          <Typography variant="body2" fontWeight={900} sx={{ color: ACCENT_COLOR }}>
            {totalProgressRate}%
          </Typography>
        </Stack>
      ...
      <Stack spacing={2.25}>
        {factor.map((f) => (
          <FactorProgress key={f.name} name={f.name} goal={f.goal} progress={f.progress} />
        ))}
      </Stack>
    </Stack>
  );
